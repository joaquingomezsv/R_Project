...
Mean_Open_Markets = mean(c(Trade.Freedom,Investment.Freedom,Financial.Freedom))
...
merged_data2 %>%
  group_by(Quartile) %>%
  ggplot() +
  geom_boxplot(aes(Quartile, Mean_Government_Size))
...
cor(merged_data2$Score, merged_data2$GDP.Billions.PPP)
...
ggplot(data=merged_data2 %>%
  ...
  geom_bar(stat='identity', aes(fill=factor(mean_score))) + xlab('Country') + ylab('Mean Score') + theme(legend.position = "none")
